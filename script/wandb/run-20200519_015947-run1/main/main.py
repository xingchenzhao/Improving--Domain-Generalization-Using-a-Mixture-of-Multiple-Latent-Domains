...
if __name__ == '__main__':
    ...
    parser.add_argument('--only-color-jitter', action='store_true')
    ...
    if args.wandb_proj_name is not None:
        import wandb
        wandb.init(project=args.wandb_proj_name, name=args.wandb_run_name, id=args.wandb_run_name,resume='allow')
    ...
    target_domain_name = target_domain[0]
    ...
    source_train, source_val, target_test = random_split_dataloader(
        ...
        color_jitter=args.color_jitter, only_color_jitter= args.only_color_jitter,min_scale=args.min_scale)
...
#     print(num_epoch)
    ...
    for epoch in range(num_epoch):
        ...
        model, optimizers = get_train(args.train)(
            ...
            disc_weight=weight, entropy_weight=args.entropy_weight, grl_weight=args.grl_weight, wandb=args.wandb_proj_name, target_domain=target_domain_name)

        if epoch % args.eval_step == 0:
            acc =  eval_model(model, source_val, device, epoch, path+'/source_eval.txt', wandb=args.wandb_proj_name, eval_domain='source', target_domain=target_domain_name)
            acc_ = eval_model(model, target_test, device, epoch, path+'/target_test.txt',  wandb=args.wandb_proj_name, eval_domain='target', target_domain=target_domain_name)
    ...
    if args.target_domain_name is not None:
        wandb.log({
            f'{target_domain_name}_test_accuracy': test_acc,
            f'{target_domain_name}_test_best_epoch': best_epoch
        })
