...
def train(model, train_data, optimizers, device, epoch, num_epoch, filename, entropy, disc_weight=None, entropy_weight=1.0, grl_weight=1.0, wandb=None, target_domain=None):
    ...
    for inputs, labels, domains in train_data:
        ...
        if entropy_weight != 0:
            ...
            loss_entropy = entropy_criterion(output_class) * 0
    ...
    if wandb is not None:
        import wandb
        wandb.log({
            f'{target_domain}_epoch': epoch,
            f'{target_domain}_train_class_loss': epoch_loss_class,
            f'{target_domain}_train_class_acc': epoch_acc_class,
            f'{target_domain}_train_domain_loss': epoch_loss_domain,
            f'{target_domain}_train_domain_acc': epoch_acc_domain,
            f'{target_domain}_train_entropy_loss': epoch_loss_entropy,
            f'{target_domain}_train_total_loss': total_loss
        })
