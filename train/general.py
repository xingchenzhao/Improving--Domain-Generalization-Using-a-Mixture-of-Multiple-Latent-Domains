...
def train(model, train_data, optimizers, device, epoch, num_epoch, filename, entropy, disc_weight=None, entropy_weight=1.0, grl_weight=1.0, vat_eps=60):
    ...
    for inputs, labels, domains in train_data:
        ...
        loss_vat = vat_criterion(inputs, output_class,
                                 domain=False)
        ...
        total_loss = loss_class + loss_domain + loss_entropy * beta + loss_vat
        total_loss.backward()
        for optimizer in optimizers:
            optimizer.step()
        ...
        running_loss_vat += loss_vat.item() * inputs.size(0)
    ...
    epoch_loss_vat = running_loss_vat / len(train_data.dataset)

    log = 'Train: Epoch: {} Alpha: {:.4f} Loss Class: {:.4f} Acc Class: {:.4f}, Loss Domain: {:.4f} Acc Domain: {:.4f} Loss Entropy: {:.4f} Loss Vat: {:.4f}'.format(
        epoch, alpha, epoch_loss_class, epoch_acc_class, epoch_loss_domain, epoch_acc_domain, epoch_loss_entropy, epoch_loss_vat)
    print(log)
